 ...
 * 纵向 上下两个方向
 */
class Direction4Rotate45Strategy implements IDealRockerMoveStrategy{
    @Override
    ...
    public void dealCallBackModeStateChange(RockerView rockerView, double angle) {
        if ((multiConditionJudgmentIsInRange(angle, ANGLE_ROTATE45_4D_OF_0P, ANGLE_ROTATE45_4D_OF_3P)) && rockerView.getTempDirection() != RockerView.Direction.DIRECTION_RIGHT) { //0-45度和315度-360度 并且当前不是右方向
            // 右
            rockerView.setTempDirection(RockerView.Direction.DIRECTION_RIGHT);
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_RIGHT);
        } else if (judgmentIsInRange(angle, ANGLE_ROTATE45_4D_OF_0P, ANGLE_ROTATE45_4D_OF_1P) && rockerView.getTempDirection() != RockerView.Direction.DIRECTION_DOWN) { //45度-135度 并且当前不是下方向
            // 下
            rockerView.setTempDirection(RockerView.Direction.DIRECTION_DOWN);
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_DOWN);
        } else if (judgmentIsInRange(angle, ANGLE_ROTATE45_4D_OF_1P, ANGLE_ROTATE45_4D_OF_2P) && rockerView.getTempDirection() != RockerView.Direction.DIRECTION_LEFT) { //135度-225度 并且当前不是左方向
            // 左
            rockerView.setTempDirection(RockerView.Direction.DIRECTION_LEFT);
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_LEFT);
        } else if (judgmentIsInRange(angle, ANGLE_ROTATE45_4D_OF_2P, ANGLE_ROTATE45_4D_OF_3P) && rockerView.getTempDirection() != RockerView.Direction.DIRECTION_UP) { //225度-315度 并且当前不是上方向
            // 上
