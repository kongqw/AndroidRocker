 ...
 * 纵向 上下两个方向
 */
class Direction8Strategy implements IDealRockerMoveStrategy{
    @Override
    public void dealCallBackModeMove(RockerView rockerView, double angle) {
        if (multiConditionJudgmentIsInRange(angle, ANGLE_8D_OF_0P, ANGLE_8D_OF_7P)) {   //0-22.5度和337.5度-360度
            ...
            // 右下
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_DOWN_RIGHT);
            ...
            // 左上
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_UP_LEFT);
    ...
    public void dealCallBackModeStateChange(RockerView rockerView, double angle) {
        ...
        if ((multiConditionJudgmentIsInRange(angle, ANGLE_8D_OF_0P, ANGLE_8D_OF_7P)) && rockerView.getTempDirection()  != RockerView.Direction.DIRECTION_RIGHT) {//0-22.5度和337.5度-360度 并且当前不是右方向
            // 右
            rockerView.setTempDirection(RockerView.Direction.DIRECTION_RIGHT);
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_RIGHT);

        } else if (judgmentIsInRange(angle, ANGLE_8D_OF_0P, ANGLE_8D_OF_1P) && rockerView.getTempDirection()  != RockerView.Direction.DIRECTION_DOWN_RIGHT) { //22.5度 - 67.5度 并且当前不是右下方向
            // 右下
            rockerView.setTempDirection(RockerView.Direction.DIRECTION_DOWN_RIGHT);
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_DOWN_RIGHT);
        } else if (judgmentIsInRange(angle, ANGLE_8D_OF_1P, ANGLE_8D_OF_2P) && rockerView.getTempDirection()  != RockerView.Direction.DIRECTION_DOWN) {// 67.5度 - 112.5度 并且当前不是下方向
            // 下
            rockerView.setTempDirection(RockerView.Direction.DIRECTION_DOWN);
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_DOWN);
        } else if (judgmentIsInRange(angle, ANGLE_8D_OF_2P, ANGLE_8D_OF_3P) && rockerView.getTempDirection()  != RockerView.Direction.DIRECTION_DOWN_LEFT) {//  112.5度 - 157.5度 并且当前不是左下方向
            // 左下
            rockerView.setTempDirection(RockerView.Direction.DIRECTION_DOWN_LEFT);
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_DOWN_LEFT);
        } else if (judgmentIsInRange(angle, ANGLE_8D_OF_3P, ANGLE_8D_OF_4P) && rockerView.getTempDirection()  != RockerView.Direction.DIRECTION_LEFT) {// 157.5度 -  202.5度 并且当前不是左方向
            // 左
            rockerView.setTempDirection(RockerView.Direction.DIRECTION_LEFT);
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_LEFT);
        } else if (judgmentIsInRange(angle, ANGLE_8D_OF_4P, ANGLE_8D_OF_5P) && rockerView.getTempDirection()  != RockerView.Direction.DIRECTION_UP_LEFT) {// 202.5度 -  247.5度 并且当前不是左上方向
            // 左上
            rockerView.setTempDirection(RockerView.Direction.DIRECTION_UP_LEFT);
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_UP_LEFT);
        } else if (judgmentIsInRange(angle, ANGLE_8D_OF_5P, ANGLE_8D_OF_6P) && rockerView.getTempDirection()  != RockerView.Direction.DIRECTION_UP) {//  247.5度 - 292.5度 并且当前不是上方向
            // 上
            rockerView.setTempDirection(RockerView.Direction.DIRECTION_UP);
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_UP);
        } else if (judgmentIsInRange(angle, ANGLE_8D_OF_6P, ANGLE_8D_OF_7P) && rockerView.getTempDirection()  != RockerView.Direction.DIRECTION_UP_RIGHT) {//  292.5度 - 337.5度 并且当前不是右上方向
            // 右上
            rockerView.setTempDirection(RockerView.Direction.DIRECTION_UP_RIGHT);
            rockerView.getOnShakeListener().direction(RockerView.Direction.DIRECTION_UP_RIGHT);
        }
